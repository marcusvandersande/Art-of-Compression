...
private:
   ...
   void pruneHelper(double pct, int tol, Node* n);

   double tolLeafCounter(Node* root, Node* n, int tol);
   ...
   * You may want a recursive helper function for this one.
   ...
   /**
   adds all leaves of the tree into a queue of node pointers accessed by
   a pointer.
   @param n is added to queue if it is a leaf node, otherwise the
   function is called recursively.
   @param q is the pointer to the queue of node pointers that is
   then used by the render function.
   */

   void renderHelper(Node* n, PNG &pic);
   ...
   */
   ...
   /** returns a copy of the Node recursively
   */
